...
export default function Separar() {  
  ...
  useEffect(() => {
    if (isProductLoaded) {
      const completScan = products.find((product) => !product.num_serie && !product.lote);
      if (!completScan) {
  ...
  function verifyItem() {    
    let item;
    const verifiyCode = products.find((product) => product.cod_prod === leitura[0] && (product.num_serie === leitura[1]))    
    if(!verifiyCode){      
      item = products.find((product) => product.cod_prod === leitura[0] && !product.num_serie && !product.lote);          
        }else{      
    ...
    if (item) {
      ...
      showToast('error', 'Leitura Invalida', 'Item não pertence a esse pedido!')     
      return
    }    
  }
